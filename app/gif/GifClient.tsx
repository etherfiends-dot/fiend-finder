...
function GifContent() {
  ...
  useEffect(() => {
    ...
    if (!encoded) { setError('No GIF data provided'); return; }
    try {
      const decoded = JSON.parse(atob(encoded));
      if (!decoded.images || decoded.images.length < 2) { setError('Invalid GIF data'); return; }
      setData(decoded);
    } catch { setError('Failed to decode GIF data'); }
  }, [searchParams]);
    ...
    <main className="bg-slate-950 min-h-screen text-white p-4">
      <div className="max-w-lg mx-auto">
        <div className="text-center mb-4 pt-4">
          <h1 className="text-xl font-black tracking-tighter text-cyan-400">NFT GIF</h1>
        </div>
        ...
        <div className="bg-black rounded-2xl overflow-hidden border border-cyan-500/30 shadow-lg shadow-cyan-500/10">
          <div className="relative aspect-square">
            <img src={data.images[currentFrame]} alt={`Frame ${currentFrame + 1}`} className="w-full h-full object-contain" />
            <div className="absolute top-3 right-3 bg-black/60 backdrop-blur-sm rounded-full px-3 py-1">
              <span className="text-cyan-400 text-sm font-mono">{currentFrame + 1}/{data.images.length}</span>
            </div>
          ...
          <div className="bg-slate-900 p-3 flex justify-center gap-2">
            {data.images.map((_, i) => (
              <div key={i} className={`w-2 h-2 rounded-full transition-all ${i === currentFrame ? 'bg-cyan-500 scale-125' : 'bg-slate-600'}`} />
            ))}
          </div>
        </div>
        ...
        <div className="mt-8 text-center">
          ...
          <a href="/" className="inline-block px-6 py-3 bg-cyan-500 hover:bg-cyan-600 text-white rounded-xl font-medium transition-colors">Make a GIF</a>
        </div>
        <div className="mt-8 text-center">
